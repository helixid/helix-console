...
describe('api/client', () => {
  ...
  describe('auth header strategy (default: admin-key only)', () => {
    it('sends the admin key on every request by default', async () => {
      ...
      expect(headersOf(init).get('x-admin-api-key')).toBe('admin-key');
      expect(headersOf(init).get('authorization')).toBeNull();
    });

    it('lets a downstream product override the auth header via setAuthHeaderStrategy', async () => {
      const { api, setAuthHeaderStrategy } = await importApi();
      setAuthHeaderStrategy({
        apply: (headers) => headers.set('authorization', 'Bearer custom-token'),
      });
      fetchMock.mockResolvedValueOnce(jsonResponse([]));
      ...
      const init = fetchMock.mock.calls[0]?.[1] as RequestInit | undefined;
      expect(headersOf(init).get('authorization')).toBe('Bearer custom-token');
    });

    it('calls handleUnauthorized on a 401 and retries once if it returns true', async () => {
      const { api, setAuthHeaderStrategy } = await importApi();
      const handleUnauthorized = vi.fn().mockResolvedValue(true);
      setAuthHeaderStrategy({ apply: () => {}, handleUnauthorized });
      fetchMock
