...
HelixID Console is the operator UI for the HelixID trust stack — it reads and acts on agent identity, credential, and revocation state through the HelixID API. It is a browser application that holds an operator session and can revoke credentials, so we treat security reports against it seriously.
...
2. **GitHub Security Advisory:** Use the "Report a vulnerability" button under the **Security** tab of the [HelixID repository](https://github.com/helixid/helix-console/security/advisories). This opens a private advisory visible only to repository maintainers.
...
- **Authentication and session handling** — operator login, session fixation, credential storage in the browser
- **Runtime configuration injection** — `API_BASE_URL` / `ADMIN_API_KEY` handling, and any path by which the admin key could reach the browser
- **Cross-site scripting** — rendering of DIDs, credential subjects, scopes, and audit payloads that originate from agents rather than operators
- **Privileged actions** — revocation and enrollment flows reachable without the intended authorization
- **Supply-chain concerns** — tampered npm artifacts and the published `helixid/console` container image
...
- **Subscribe to advisories:** Watch this repository with "Releases and security advisories" enabled, or subscribe via RSS to the [Security Advisories feed](https://github.com/helixid/helix-console/security/advisories).
...
- [Revocation](https://docs.helixid.dev/concepts/revocation) — what the Console's revoke action actually does
...
- **GitHub Security Advisories:** [github.com/helixid/helix-console/security/advisories/new](https://github.com/helixid/helix-console/security/advisories/new)
